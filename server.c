...
int main (void) {
  ...
  while (1) {
    // Recebe conexão
    if(MAX_CLIENTS > clients_conec){
      client_s = accept(s, (struct sockaddr*)&client, &addr_len);
      clients_conec = clients_conec ++;

      pthread_create(&id[clients_conec],NULL,multUser,(void*)client_s);
      //pthread_join(id[clients_conec], NULL);

    }else{
      printf(" Conexão máxima estabelecida \n");
    }
  }
